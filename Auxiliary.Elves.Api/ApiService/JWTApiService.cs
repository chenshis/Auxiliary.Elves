...
namespace Auxiliary.Elves.Api.ApiService
{
    public class JWTApiService : IJWTApiService
    {
        ...
        public JWTApiService( ILogger<JWTApiService> logger,AuxiliaryDbContext auxiliaryDbContext)
        {
            _logger = logger;
            _dbContext = auxiliaryDbContext;
        }
        ...
        /// <returns></returns>
        public string GetToken(UserServerEntity userEntity)
        {
        ...
        string IJWTApiService.RefreshToken(string oldToken)
        {
            ...
            var userEntity = _dbContext.UserServerEntities.FirstOrDefault(t => t.Jti == jti);
            if (userEntity == null)
            {
                throw new Exception(SystemConstant.ErrorRefreshTokenFailMessage);
            }
